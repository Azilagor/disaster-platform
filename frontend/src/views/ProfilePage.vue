  ...
  <div class="profile-page">
      ...
      <div class="profile-info-section">
          ...
          >
            ...
            <template v-else>
              <div class="profile-avatar-placeholder">
                ...
                <span class="avatar-placeholder-text">{{ $t('profile.photo') }}</span>
              </div>
            </template>
            <input
              ref="avatarInputRef"
              ...
              :aria-label="$t('profile.photoAria')"
        ...
        <div class="profile-header-info">
          ...
          <div class="profile-actions">
            <button type="button" class="btn btn-primary btn-sm" @click="openEditProfileModal">{{ $t('profile.editProfile') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="profile-content">
      <div class="profile-left-column">
        <div class="card">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.personal') }}</h2>
          </div>
          <form class="profile-form" @submit.prevent="saveProfile">
            <div class="form-grid">
              ...
              <div class="form-group">
                <label for="profile-phone">{{ $t('common.phone') }}</label>
                <input
                  id="profile-phone"
              ...
              <div class="form-group">
                <label for="profile-district">{{ $t('common.district') }}</label>
            ...
            <div class="form-actions">
              <button type="submit" class="btn btn-primary" :disabled="profileSaving">{{ $t('common.save') }}</button>
            </div>
          </form>
        ...
        <div class="card">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.statistics') }}</h2>
          </div>
          <div class="profile-stats" style="padding: var(--spacing-xl)">
            <div class="profile-stat-item">
              ...
              <div class="stat-content">
                ...
                <div class="stat-label">{{ $t('profile.requestsCreated') }}</div>
              </div>
            </div>
            <div class="profile-stat-item">
              ...
              <div class="stat-content">
                ...
                <div class="stat-label">{{ $t('profile.doneAsVolunteer') }}</div>
              </div>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.myRequests') }}</h2>
          </div>
          <div v-if="myRequestsLoading" class="profile-requests-loading">{{ $t('common.loading') }}</div>
          ...
          <div v-else-if="myRequests.length === 0" class="profile-requests-empty">
            {{ $t('profile.noRequestsYet') }}
            <router-link to="/create-request">{{ $t('nav.createRequest') }}</router-link>
          </div>
            ...
            <li v-for="req in myRequests" :key="req.id" class="profile-request-item">
              <router-link :to="'/map?request=' + req.id" class="profile-request-link">
                ...
                <span class="profile-request-meta">
                  {{ $t('enums.problemType.' + req.problemType) }} ·
                  {{ $t('enums.priority.' + req.priority) }} ·
        ...
        <div class="card">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.skills') }}</h2>
          </div>
          <div style="padding: var(--spacing-xl)">
            <div class="skills-grid">
              <span v-for="k in skillKeys" :key="k" class="skill-badge-large">{{ $t(k) }}</span>
            </div>
          </div>
        </div>
        <div class="card card-danger-zone">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.deleteAccount') }}</h2>
          </div>
          <div class="danger-zone-content">
            ...
            <form class="delete-account-form" @submit.prevent="submitDeleteAccount">
              <div class="form-group">
                <label for="delete-password">{{ $t('profile.deletePasswordLabel') }}</label>
                <input
                  id="delete-password"
              ...
              <label class="checkbox-label">
                ...
                <span>{{ $t('profile.deleteUnderstand') }}</span>
              </label>
              <button
                type="submit"
              ...
              >
                {{ deleteAccountSaving ? $t('profile.deleting') : $t('profile.deleteSubmit') }}
              </button>
            </form>
          </div>
        </div>
      ...
      <div class="profile-right-column">
        <div class="card">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.activity') }}</h2>
          </div>
        ...
        <div class="card">
          <div class="card-header">
            <h2 class="card-title">{{ $t('profile.achievements') }}</h2>
          </div>
    ...
    <div v-if="editProfileModalOpen" class="modal-overlay" @click.self="editProfileModalOpen = false">
      <div class="modal-card">
        <div class="modal-header">
          <h2 class="modal-title">{{ $t('profile.editModalTitle') }}</h2>
        ...
        <form class="modal-body" @submit.prevent="saveEditProfile">
          <p class="modal-hint">{{ $t('profile.editModalHint') }}</p>
          <div class="form-group">
            <label for="edit-profile-email">{{ $t('common.email') }}</label>
            <input
...
async function onAvatarFileChange(e) {
  ...
  try {
    ...
    avatarError.value = err.message || t('profile.avatarUploadError')
  } finally {
...
async function saveEditProfile() {
  ...
  if (!email) {
    editProfileError.value = t('profile.emailRequired')
    return
  }
  editProfileError.value = ''
  editProfileSuccess.value = ''
  ...
  try {
    ...
    editProfileSuccess.value = t('profile.emailSaved')
    setTimeout(() => {
      editProfileModalOpen.value = false
      editProfileSuccess.value = ''
    }, 2500)
  } catch (e) {
    editProfileError.value = e.message || t('profile.emailSaveError')
  } finally {
...
async function submitDeleteAccount() {
  ...
  try {
    ...
    deleteAccountError.value = err.message || t('profile.deleteError')
  } finally {
...
async function saveProfile() {
  ...
  try {
    ...
    profileError.value = e.message || t('profile.profileSaveError')
  } finally {
...
const skillKeys = ['profile.skill1', 'profile.skill2']

const timeline = computed(() => [
  {
    id: 1,
    type: 'success',
    text: t('profile.timeline1'),
    time: t('profile.timeline1t'),
  },
  {
    id: 2,
    type: 'info',
    text: t('profile.timeline2'),
    time: t('profile.timeline2t'),
  },
  {
    id: 3,
    type: 'warning',
    text: t('profile.timeline3'),
    time: t('profile.timeline3t'),
  },
])

const achievements = [
  { id: 1, titleKey: 'profile.ach1Title', descKey: 'profile.ach1Desc', unlocked: true },
  { id: 2, titleKey: 'profile.ach2Title', descKey: 'profile.ach2Desc', unlocked: true },
  { id: 3, titleKey: 'profile.ach3Title', descKey: 'profile.ach3Desc', unlocked: false },
]
...
onMounted(async () => {
  ...
  try {
    ...
    myRequestsError.value = e.message || t('profile.requestsLoadError')
  } finally {
