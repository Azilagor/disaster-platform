      ...
      <div class="request-cards">
        <div v-for="r in availableRequests" :key="r.id" class="request-card">
          <div class="request-card-header">
            <span class="priority-badge" :class="(r.priority || '').toLowerCase()">{{ $t('enums.priority.' + r.priority) }}</span>
            <span class="problem-type">{{ $t('enums.problemType.' + r.problemType) }}</span>
          </div>
          <h3>{{ r.title }}</h3>
          <p class="request-address">{{ r.address }}</p>
          <p class="request-district">{{ $t('enums.district.' + r.district) }}</p>
          <p v-if="r.peopleCount" class="request-meta">{{ $t('volunteerTasks.people', { n: r.peopleCount }) }}</p>
          <div class="request-card-actions">
            ...
            >
              {{ respondingId === r.id ? '...' : $t('map.respond') }}
            </button>
            <span v-else class="already-responded-badge">{{ $t('volunteerTasks.alreadyResponded') }}</span>
          </div>
        </div>
      </div>
      <p v-if="availableLoading" class="text-muted">{{ $t('common.loading') }}</p>
    ...
    <div v-show="tab === 'assigned'" class="section">
      ...
      <div class="request-cards">
        <div v-for="r in assignedRequests" :key="r.id" class="request-card">
          <div class="request-card-header">
            <span class="priority-badge" :class="(r.priority || '').toLowerCase()">{{ $t('enums.priority.' + r.priority) }}</span>
            <span class="status-badge">{{ $t('enums.requestStatus.' + r.status) }}</span>
          </div>
          <h3>{{ r.title }}</h3>
          <p class="request-address">{{ r.address }}</p>
          <p class="request-district">{{ $t('enums.district.' + r.district) }}</p>
          <div class="request-card-actions">
            ...
            >
              {{ leavingId === r.id ? '...' : $t('volunteerTasks.leave') }}
            </button>
          </div>
        </div>
      </div>
      <p v-if="assignedLoading" class="text-muted">{{ $t('common.loading') }}</p>
