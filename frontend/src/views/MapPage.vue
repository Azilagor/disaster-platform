    ...
    <div class="map-container">
      <div class="map-header">
        ...
        >
          ...
          <span class="panel-toggle-label">{{ panelOpen ? $t('common.hideFilters') : $t('common.filters') }}</span>
        </button>
        <div class="search-box">
          <input v-model="searchQuery" type="text" :placeholder="$t('map.searchPlaceholder')" class="form-control" />
        </div>
        <router-link v-if="canCreateRequest" to="/create-request" class="btn btn-primary">{{ $t('map.createRequest') }}</router-link>
      </div>
      <div class="map-area">
        ...
        <div class="map-legend">
          ...
          <div class="legend-items">
            <div class="legend-item">
              <span class="legend-marker critical-marker"></span><span>{{ $t('enums.priority.CRITICAL') }}</span>
            </div>
            <div class="legend-item">
              <span class="legend-marker high-marker"></span><span>{{ $t('enums.priority.HIGH') }}</span>
            </div>
            <div class="legend-item">
              <span class="legend-marker medium-marker"></span><span>{{ $t('enums.priority.MEDIUM') }}</span>
            </div>
            <div class="legend-item">
              <span class="legend-marker low-marker"></span><span>{{ $t('enums.priority.LOW') }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <aside class="filters-drawer" :class="{ 'filters-drawer--open': panelOpen }" aria-hidden="!panelOpen">
      ...
      <div class="filter-group">
        ...
        <select v-model="filters.scope" class="form-control">
          ...
          <option value="mine">{{ $t('map.mineRequests') }}</option>
        </select>
      </div>
      <div class="filter-group">
        <span class="filter-label">{{ $t('common.district') }}</span>
      ...
      <div class="filter-group">
        <span class="filter-label">{{ $t('common.priority') }}</span>
      ...
      <div class="filter-group">
        <span class="filter-label">{{ $t('coordinatorRequests.problemType') }}</span>
        <select v-model="filters.problemType" class="form-control">
          <option value="">{{ $t('map.allTypes') }}</option>
          <option v-for="pt in ALLOWED_PROBLEM_TYPES" :key="pt" :value="pt">{{ $t('enums.problemType.' + pt) }}</option>
        </select>
      </div>
      <div class="filter-stats">
        <div class="filter-stat-item">
          <strong>{{ requests.length }}</strong>
          <span>{{ $t('map.onMap') }}</span>
        </div>
      </div>
    ...
    <div v-if="selectedRequest" class="modal" @click.self="selectedRequest = null">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h2>{{ $t('map.requestN', { n: selectedRequest.id }) }}</h2>
        ...
        <div class="modal-body">
          <div class="request-detail-header">
            <span class="priority-badge" :class="(selectedRequest.priority || '').toLowerCase()">
              {{ $t('enums.priority.' + (selectedRequest.priority || 'MEDIUM')) }}
            </span>
            <span class="problem-type">{{ $t('enums.problemType.' + (selectedRequest.problemType || 'MEDICAL')) }}</span>
          </div>
        ...
        <div class="modal-footer">
          ...
          >
            {{ respondingId === selectedRequest.id ? '...' : $t('map.respond') }}
          </button>
        </div>
      </div>
    </div>
...
function updateMarkers() {
  ...
  withCoords.forEach(({ r, pos }) => {
    ...
    marker.request = r
    marker.bindTooltip(escapeHtml(r.title || t('common.request')), {
      direction: 'top',
      permanent: false,
      offset: [0, -36],
    ...
    if (fullDetail) {
      ...
      const popupContent =
        ...
        '<strong>' + escapeHtml(r.title || t('common.request')) + '</strong>' +
