      ...
      <div class="filter-group">
        ...
        <div class="checkbox-group">
          <label class="checkbox-item">
            <input type="checkbox" v-model="filterStatusNew" />
            <span class="checkbox-text"><span class="status-indicator status-new"></span> Новый</span>
          </label>
          <label class="checkbox-item">
            <input type="checkbox" v-model="filterStatusInProgress" />
            <span class="checkbox-text"><span class="status-indicator status-in-progress"></span> В работе</span>
          </label>
          <label class="checkbox-item">
            <input type="checkbox" v-model="filterStatusCompleted" />
            <span class="checkbox-text"><span class="status-indicator status-completed"></span> Выполнен</span>
          </label>
        </div>
      </div>
      <div class="filter-group">
        ...
        <div class="checkbox-group">
          <label v-for="p in filterPriorities" :key="p.id" class="checkbox-item">
            ...
            <span class="checkbox-text"><span class="priority-badge" :class="p.id">{{ p.label }}</span></span>
          </label>
        </div>
      </div>
    ...
    <div class="map-container">
      <div class="map-header">
        <div class="search-box">
          ...
          <input type="text" v-model="searchQuery" placeholder="Поиск по адресу или описанию..." />
        </div>
      ...
      <div class="map-placeholder">
        <div class="map-overlay">
          ...
          <p>Карта запросов</p>
          <p class="map-note">Здесь будет отображаться интерактивная карта с маркерами запросов. Подключите карту (Leaflet, Yandex.Maps или др.) для отображения точек.</p>
        </div>
      </div>
    ...
    <div v-if="selectedRequest" class="modal" @click.self="selectedRequest = null">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button type="button" class="modal-close" @click="selectedRequest = null" aria-label="Закрыть">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 2L2 18M2 2l16 16"/></svg>
          </button>
        </div>
        <div class="modal-body">
          <div class="request-detail-header">
            <span class="priority-badge" :class="selectedRequest.priority">{{ selectedRequest.priorityLabel }}</span>
...
const requests = ref([
  { id: 1247, title: 'Требуется медицинская помощь', location: 'г. Алматы, ул. Абая, 150', description: 'Пожилой человек, нужны лекарства и осмотр.', contact: '+7 777 123-45-67', priority: 'critical', priorityLabel: 'Критический', status: 'new' },
  { id: 1248, title: 'Нужна эвакуация', location: 'г. Алматы, мкр. Самал-2', description: 'Семья из 4 человек, есть дети.', contact: '+7 701 234-56-78', priority: 'high', priorityLabel: 'Высокий', status: 'in-progress' },
  { id: 1249, title: 'Продукты и вода', location: 'г. Алматы, ул. Толе би, 59', description: '2 взрослых, нужны продукты на 3 дня.', contact: '+7 705 345-67-89', priority: 'medium', priorityLabel: 'Средний', status: 'new' }
])
