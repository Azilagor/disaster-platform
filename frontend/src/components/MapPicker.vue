<template>
  <div class="map-picker">
    ...
    <div class="map-picker__hint">
      <template v-if="geocoding">🔍 Определяем адрес...</template>
      <template v-else-if="modelValue.lat && modelValue.lng">
        📍 {{ geocodedAddress || `${modelValue.lat.toFixed(5)}, ${modelValue.lng.toFixed(5)}` }}
      </template>
      <template v-else>Кликните по карте чтобы указать местоположение</template>
    </div>
...
.map-picker__header { display: flex; align-items: center; justify-content: space-between; }
.map-picker__label { font-size: 0.9rem; font-weight: 500; color: #374151; }
.map-picker__clear { font-size: 0.8rem; color: #6b7280; background: none; border: none; cursor: pointer; text-decoration: underline; padding: 0; }
.map-picker__clear:hover { color: #dc2626; }
.map-picker__hint { font-size: 0.82rem; color: #6b7280; min-height: 1.2em; }
.map-picker__map { height: 280px; border-radius: 10px; border: 1px solid #e5e7eb; overflow: hidden; cursor: crosshair; }
</style>